...
model_choice = st.sidebar.selectbox("Select your model here 🤖✨", ("llama3", "codellama"))
...
if st.sidebar.button("Generate Code"):
    with st.spinner("Generating code..."):
        st.session_state["messages"].append({"role": "user", "content": "Generate Full solution code in TypeScript and provide an explanation."})
        st.session_state["full_message"] = ""
        for token in generate_response():
            pass  # We just need to run through the generator to collect the full message
